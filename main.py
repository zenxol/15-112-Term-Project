...
def saveGameData(app): # Perplexity.ai to assist write this function and json file
    data = {
        "highScores": [int(score) for score in app.top5Scores], # Ensure integers
...
def updateTop5Scores(app):
    score = int(app.currentScoreCount)
...
def updateScore(app):
    ...
    if app.gameMode == 'Easy':
        app.currentScoreCount = rounded(app.currentScoreCount + app.difficultyFactor * 0.25) # Round to int instead of float
    elif app.gameMode == 'Medium':
        app.currentScoreCount = rounded(app.currentScoreCount + app.difficultyFactor * 0.5)
    elif app.gameMode == 'Hard':
        app.currentScoreCount = rounded(app.currentScoreCount + app.difficultyFactor * 0.6)
...
def drawTop5Scores(app):
    for i in range(len(app.top5Scores)):
        drawLabel(int(app.top5Scores[i]), 140, i * 68 + 165, size=45, bold=True, border='black', borderWidth=2, fill='white', font='orbitron' ) 
...
def drawCounters(app):
    ...
    drawLabel(int(app.currentScoreCount), 360, 50, size=20, bold=True, fill='white', border='black', borderWidth=1, font='riffic', align='center')
